...
- **首屏**：品牌、一句话介绍、跳到「试一试」的 CTA，以及「首发 / 接下来」两行上架地区说明。目前两个平台都还没上架，所以首屏没有商店下载按钮；地区文案在 `app.js` 的 `regionsNow` / `regionsNext`（六种语言各一份，`index.html` 里还有一份简中默认标记），真上架那天记得把「即将」改掉。
- **功能**：九张功能卡片（`feat1`–`feat9`）。
- **试一试**：迷你家族树模拟器 — 点人看称谓、添孩子/伴侣、右上角换四种风格（晨白/夜潮/暖橘/雅纸，颜色与 App 完全一致）。
- **联系**：邮箱按钮（现在指向 zuyingapp@gmail.com，要换地址就改 `index.html` 里的 `mailto:` 链接和显示文字）。
